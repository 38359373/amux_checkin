const endpoint = "https://elysiver.h-e.top/api/user/checkin";
...
const accessToken = process.env.ELYSIVER_ACCESS_TOKEN?.trim();
const userId = process.env.ELYSIVER_USER_ID?.trim();

if (!userId) {
  console.error("Missing ELYSIVER_USER_ID.");
  process.exit(1);
}

if (!accessToken) {
  console.error("Missing ELYSIVER_ACCESS_TOKEN.");
  process.exit(1);
}
...
function describeFailure(response, payload, message, rawBody) {
  ...
  if (response.status === 401) {
    return "Authentication failed. Check ELYSIVER_ACCESS_TOKEN and ELYSIVER_USER_ID.";
  }
  ...
  if (
    ...
    return "User identity check failed. Verify ELYSIVER_USER_ID matches the token owner.";
  }
  ...
  if (response.status >= 500) {
    return "Upstream server error from elysiver.h-e.top.";
  }
...
console.log(`ELYSIVER check-in HTTP ${response.status}`);
if (message) {
