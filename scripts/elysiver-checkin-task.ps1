...
$logFile = Join-Path $logDir "elysiver-checkin.log"
$scriptPath = Join-Path $PSScriptRoot "elysiver-checkin.js"
$configPath = Join-Path $repoRoot "config\elysiver-accounts.json"
...
function Get-Accounts {
  ...
  $accessToken = Get-EnvValue -Name "ELYSIVER_ACCESS_TOKEN"
  $userId = Get-EnvValue -Name "ELYSIVER_USER_ID"

  if ([string]::IsNullOrWhiteSpace($accessToken)) {
    throw "Missing ELYSIVER_ACCESS_TOKEN."
  }

  if ([string]::IsNullOrWhiteSpace($userId)) {
    throw "Missing ELYSIVER_USER_ID."
  }
...
function Invoke-CheckinForAccount {
  ...
  $env:ELYSIVER_ACCESS_TOKEN = $Account.AccessToken
  $env:ELYSIVER_USER_ID = $Account.UserId
...
$originalAccessToken = $env:ELYSIVER_ACCESS_TOKEN
$originalUserId = $env:ELYSIVER_USER_ID

Write-Log "Starting ELYSIVER check-in task."
$overallExitCode = 0
...
try {
  ...
  $env:ELYSIVER_ACCESS_TOKEN = $originalAccessToken
  $env:ELYSIVER_USER_ID = $originalUserId
}

Write-Log "ELYSIVER check-in task finished with exit code $overallExitCode."
